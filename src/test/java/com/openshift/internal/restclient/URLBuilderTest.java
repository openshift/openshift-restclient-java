...
public class URLBuilderTest extends TypeMapperFixture {
    ...
    public void testBuildingURLForAWatchService() throws Exception {
        ...
        String url = builder.resource(resource).watch().addParmeter("resourceVersion", "123").addParameters(params)
                .build().toString();
        assertEquals(
    ...
    public void testDuplicateParameters() throws Exception {
        ...
        assertEquals(
                String.format("%s/api/v1/namespaces/foo/services?watch=true&resourceVersion=123&x=1&x=2", BASE_URL),
                url.toString());
    }
    ...
    public void testAddingASubContext() {
        ...
        assertEquals(String.format(
                "%s/api/v1/namespaces/https:demo-app-8-3gehi:8778/pods/bar/proxy/jolokia/exec/java.util.logging:type=Logging/getLoggerLevel/abc",
                BASE_URL), url.toString());
    }

    private String whenBuildingTheURLFor(IResource resource, String namespace) {
        return builder.resource(resource).namespace(namespace).name("bar").build().toString();
    }
