	...
	private void runTest() throws Exception {
		watch = client.watch(project.getName(), new IOpenShiftWatchListener() {
			...
			public void received(IResource resource, ChangeType change) {
				if(ChangeType.MODIFIED.equals(change) && !resource.getName().endsWith("deploy") && conditional.isReady(resource)) {
		...
		if(initializationLatch.await(1, TimeUnit.MINUTES)){
			scaleTo(REPLICAS);
			assertTrue("The pods either did not scale as expected or the test timed out", replicaLatch.await(2, TimeUnit.MINUTES));
		};
