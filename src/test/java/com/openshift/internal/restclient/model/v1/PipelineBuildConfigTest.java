...
public class PipelineBuildConfigTest {
    ...
    private IBuildConfigBuilder createBuilder() throws MalformedURLException {
        ...
        return new BuildConfigBuilder(client).named("foor").inNamespace("bar");
    }
    ...
    public void shouldHaveEnvVars() {
        ...
        assertThat(strategy.getEnvVars()).containsOnly(
                new EnvironmentVariable(new ModelNodeBuilder().set("name", "foo").set("value", "bar").build(), null),
                new EnvironmentVariable(new ModelNodeBuilder().set("name", "kung").set("value", "foo").build(), null));
    }
    ...
    public void shouldHaveEnvVarsThatWereSet() {
        // given
        IJenkinsPipelineStrategy strategy = pipelineBc.getBuildStrategy();
        IEnvironmentVariable envVar = new EnvironmentVariable(
                new ModelNodeBuilder().set("name", "gargamel").set("value", "crazyLad").build(), null);
    ...
    public void shouldBuildJenkinsPipelineStrategyBuildConfig() {
        // given
        // when
        IBuildConfig bc = builder.usingJenkinsPipelineStrategy().end().build();
        // then
    ...
    public void shouldBuildJenkinsPipelineStrategyWithJenkinsfileAndPath() {
        // given
        // when
        IBuildConfig bc = builder.usingJenkinsPipelineStrategy().usingFile("node('aNode') {}")
                .usingFilePath("some/path/to/some/location").end().build();
        // then
