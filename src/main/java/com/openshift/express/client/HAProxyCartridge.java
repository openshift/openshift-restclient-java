...
public class HAProxyCartridge extends Cartridge {
	...
	public HAProxyCartridge(String name) {
		super(name);
	}
	
	public HAProxyCartridge(IOpenShiftService service, IUser user) throws OpenShiftException {
		super(service, user);
		
		name = this.getCartridgeName(HAPROXY);
	}
